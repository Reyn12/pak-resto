...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT * FROM kasir WHERE namaKasir = '$username' AND passwordKasir = '$password'"; 
    $result = $conn->query($sql); 
    
    if ($result->num_rows > 0) { 
        $_SESSION['username'] = $username; 
        session_regenerate_id(true); // Regenerate session ID to prevent fixation
        header("Location: ../dashboard/kasir/pembayaran.php"); 
        exit();
    } else {
    ...
    <div class="flex w-3/4 max-w-4xl bg-white shadow-lg overflow-hidden relative rounded-2xl">
        ...
        <div class="w-1/2 p-10">
            <div class="flex flex-col items-center">
                ...
                <h1 class="text-2xl font-bold mb-2">Log In Kasir</h1>
