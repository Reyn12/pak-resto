...
<body class="bg-black flex flex-col items-center justify-center h-screen">
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg w-3/4">
        <div class="text-center mb-8">
            <h1 class="text-2xl font-bold text-green-800">Menu Minuman</h1>
        </div>
                ...
                if ($result) {
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<td class='border'><div class='flex items-center justify-center'><img src='../../../assets/images/drink.png' alt='".$row["namaMinuman"]."' class='w-10 h-10 mr-2'>".$row["namaMinuman"]."</div></td>";
                            echo "<td class='border'>Rp. ".number_format($row["harga"], 0, ',', '.')."</td>";
                            echo "<td class='border'>".($row["ketersediaan"] > 0 ? "Ada" : "Habis")."</td>";
                            echo "<td class='border'><div class='flex justify-center items-center space-x-2'><button class='bg-red-500 text-white px-3 py-1 rounded' onclick='decreaseQuantity( \"".$row["namaMinuman"]."\")'>X</button><button class='bg-green-500 text-white px-3 py-1 rounded' onclick='increaseQuantity(\"".$row["idMenu"]."\", \"".$row["namaMinuman"]."\", \"Rp. ".number_format($row["harga"], 0, ',', '.')."\")'>+</button></div></td>";
                            echo "</tr>";
                        }
                    } else {
    ...
    function showOrder() {
        ...
        for (const [item, details] of Object.entries(orders)) {
            const total = details.quantity * parseInt(details.price.replace('Rp. ', '').replace(/\./g, ''));
            totalAmount += total;
            const tr = document.createElement('tr');
            tr.innerHTML =
                `<td>${details.name}</td><td>${details.price}</td><td>${details.quantity}</td><td>Rp. ${total.toLocaleString('id-ID')}</td>`;
    ...
    function increaseQuantity(idMenu, itemName, itemPrice) {
        const orders = JSON.parse(localStorage.getItem('orders')) || {};
        if (!orders[idMenu]) {
            orders[idMenu] = {
                id: idMenu,
            ...
            orders[idMenu].quantity += 1;
        }
        localStorage.setItem('orders', JSON.stringify(orders));
        updateOrderButton();
    }

    function decreaseQuantity(idMenu) {
        const orders = JSON.parse(localStorage.getItem('orders')) || {};
        if (orders[idMenu]) {
            orders[idMenu].quantity -= 1;
            if (orders[idMenu].quantity <= 0) {
                delete orders[idMenu];
            }
            localStorage.setItem('orders', JSON.stringify(orders));
            updateOrderButton();
        }
