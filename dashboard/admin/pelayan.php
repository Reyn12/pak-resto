...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql_insert = "INSERT INTO pelayan (idPelayan, namaPelayan, passwordPelayan) VALUES ('$id', '$nama', '$password')";
...
// Query untuk mengambil data pelayan
$sql = "SELECT idPelayan, namaPelayan, passwordPelayan FROM pelayan";
...
<body class="flex bg-white">
    ...
    <div class="flex-1 p-10 text-xl font-bold">
        <div class="flex justify-between items-center mb-5">
            <h2 class="text-gray-700 text-base">Pelayan</h2>
            <button id="addWaiterButton" class="bg-green-500 text-white text-sm px-4 py-2 rounded">Tambah
                Pelayan</button>
        </div>
        <table id="waiterTable" class="min-w-full bg-white border rounded">
            <thead>
                <tr>
                    <th class="py-2 px-4 border-b text-sm text-left">No</th>
                    <th class="py-2 px-4 border-b text-sm text-left">ID</th>
                    <th class="py-2 px-4 border-b text-sm text-left">Nama Pelayan</th>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo "<td class='py-2 px-4 border-b font-normal text-sm text-left'>" . $row["idPelayan"] . "</td>";
                        echo "<td class='py-2 px-4 border-b font-normal text-sm text-left'>" . $row["namaPelayan"] . "</td>";
                        echo "<td class='py-2 px-4 border-b font-normal text-sm text-left'>" . $row["passwordPelayan"] . "</td>";
                        echo "</tr>";
                    }
                } else {
    ...
    <!-- Form "Tambah Pelayan" -->
    <div id="addWaiterForm" class="hidden fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white p-8 rounded-lg shadow-lg max-w-lg w-full">
            <h3 class="text-2xl mb-4">Tambah Pelayan</h3>
            <form id="waiterForm">
                <div class="mb-4">
                ...
                <div class="mb-4">
                    <label for="nama" class="block text-sm font-medium text-gray-700">Nama Pelayan</label>
    ...
    <div id="successAlert" class="hidden fixed inset-0 flex items-center justify-center">
        <div class="bg-white p-8 rounded-lg shadow-lg text-center">
            ...
            <p class="text-xl">PELAYAN BERHASIL DITAMBAHKAN!!!</p>
    ...
    const addWaiterButton = document.getElementById("addWaiterButton");
    ...
    const waiterTable = document.getElementById("waiterTable").querySelector("tbody");

    addWaiterButton.addEventListener("click", () => {
        addWaiterForm.classList.remove("hidden");
        addWaiterForm.classList.add("animate-fade-in");
    });

    cancelButton.addEventListener("click", () => {
        addWaiterForm.classList.remove("animate-fade-in");
        addWaiterForm.classList.add("animate-fade-out");
        setTimeout(() => {
            addWaiterForm.classList.add("hidden");
            addWaiterForm.classList.remove("animate-fade-out");
        }, 300);
    });
    ...
    waiterForm.addEventListener("submit", (e) => {
        ...
        xhr.open("POST", "pelayan.php", true);
        ...
        xhr.onload = function() {
            if (xhr.status === 200 && xhr.responseText.trim() === "success") {
                ...
                newRow.innerHTML = `
                    ...
                    `;
                waiterTable.appendChild(newRow);
                ...
                console.error("Error adding waiter: ", xhr.responseText);
            }
        };
